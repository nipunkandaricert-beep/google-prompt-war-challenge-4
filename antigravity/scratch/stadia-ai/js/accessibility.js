(function() {
  ...
  StadiaAI.Accessibility = {
    ...
    renderFeatures: function() {
      ...
      features.forEach(f => {
        ...
        card.innerHTML = `
          ...
          <p class="text-secondary text-sm" style="line-height:1.4;">${f.desc}</p>
          <span class="text-gold text-xs font-bold" style="margin-top:auto; padding-top:var(--space-2)">📍 ${f.location}</span>
        `;
    ...
    populateSelects: function() {
      ...
      zones.forEach(z => {
        html += `<option value="${z.id}">${z.name} (L${z.level})</option>`;
      });
