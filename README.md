...
The project ships a zero-dependency unit test suite (`tests/utils.test.js`,
`tests/data.test.js`) using Node's built-in test runner — no `npm install`
required to run the tests themselves. It covers the pure utility and
data-layer functions, including an XSS-regression test for `escapeHtml` and
boundary tests for the shared crowd-density classifier.
...
Requires Node.js 18+. Tests and linting both run automatically in CI on
every push and pull request via `.github/workflows/tests.yml` (Node 18.x
and 20.x).
...
ESLint (`eslint.config.js`) catches real bugs — undefined globals, unused
variables, accidental `var`/loose-equality usage — rather than enforcing a
style guide, since this is a build-step-free vanilla JS project. Crowd
density classification (the 50/70/85% severity breakpoints) used to be
duplicated with slightly different scales across `crowd.js` and
`stadium-map.js`; it's now a single `StadiaAI.Utils.getDensityLevel()`
function with its own boundary tests.
...
- **XSS:** every piece of dynamic text rendered via `innerHTML` — chat
  messages, streamed and non-streamed Gemini responses, zone/alert/transport/
  accessibility data, match schedule data — is passed through
  `StadiaAI.Utils.escapeHtml()`. This was audited file-by-file, not just
  applied to the obvious chat input.
- **API key lifetime:** the Gemini key defaults to `sessionStorage` (cleared
  when the tab closes) and is only written to `localStorage` if the user
  explicitly checks "Remember this key on this device." A "Forget stored
  key" button clears both. Sent via the `x-goog-api-key` header, never in
  the URL.
- **CSP:** locked down beyond the basics — `object-src 'none'`,
  `base-uri 'self'`, `form-action 'self'`, `frame-ancestors 'none'`,
  `worker-src 'none'`, `upgrade-insecure-requests`, plus the original
  `script-src 'self'` / restricted `connect-src`.
- **Referrer-Policy:** `strict-origin-when-cross-origin`, so the full page
  URL isn't leaked to Google Fonts or the Gemini API.
- **External links:** `rel="noopener noreferrer"` on the one external link
  in the app (reverse-tabnabbing protection).
- **Automated scanning:** CodeQL + Dependabot run on every push via
