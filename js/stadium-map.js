  ...
  StadiaAI.StadiumMap = {
    ...
    updateDensity: function(zones) {
      zones.forEach(zone => {
        ...
        const level = StadiaAI.Utils.getDensityLevel(density * 100);
        if (level === 'critical') {
    ...
    selectZone: function(zoneId, refocus = true) {
      ...
      const progressColor = level === 'critical'
        ? 'var(--accent-red)'
        : level === 'high'
          ? 'var(--accent-orange)'
          : 'var(--accent-teal)';
        ...
        concessionsHtml = concessions.map(c => `
          <div class="concession-item" style="margin-top: 8px; padding-bottom: 8px; border-bottom: 1px dashed var(--border-subtle)">
            <div style="display:flex; justify-content:space-between; align-items:center;">
              <strong>${StadiaAI.Utils.escapeHtml(c.name)}</strong>
            ...
            <p class="text-secondary text-sm" style="margin-top: 2px;">
              Popular: ${StadiaAI.Utils.escapeHtml(c.items.filter(i => i.popular).map(i => i.name).join(', '))}
            </p>
          </div>
        `).join('');
      }
      ...
      panel.innerHTML = `
        <div style="display:flex; flex-direction:column; gap: var(--space-4); margin-top: var(--space-4);">
          <div>
            <h4 style="font-size: 1.125rem;">${StadiaAI.Utils.escapeHtml(zone.name)}</h4>
            <span class="text-muted text-sm" style="text-transform: capitalize;">Type: ${StadiaAI.Utils.escapeHtml(zone.type)} • Level ${zone.level}</span>
          </div>
          ...
          <div class="progress-bar">
            <div class="progress-fill" style="width: ${density}%; background-color: ${progressColor}"></div>
          </div>
          ...
          <div style="display:flex; gap:var(--space-2); margin-top:var(--space-4);">
            <button class="btn btn-primary btn-sm btn-nav-from" data-zone="${StadiaAI.Utils.escapeHtml(zone.id)}" style="flex:1;">From Here</button>
            <button class="btn btn-secondary btn-sm btn-nav-to" data-zone="${StadiaAI.Utils.escapeHtml(zone.id)}" style="flex:1;">To Here</button>
          </div>
        </div>
      `;
