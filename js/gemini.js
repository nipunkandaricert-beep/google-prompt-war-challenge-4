...
Analyze the alert, provide a 3-step immediate action plan, specify the dispatch priority (Low/Medium/High/Critical), and recommend resource deployment. Keep it tactical, concise, and executive-level.`,
  ...
  StadiaAI.Gemini = {
    ...
    init: function() {
      // Session storage is the default (cleared when the tab closes).
      // Fall back to localStorage only if the user previously opted in
      // via the "remember this key" checkbox.
      this.apiKey = StadiaAI.Utils.sessionStorage.get('stadia_api_key')
        || StadiaAI.Utils.storage.get('stadia_api_key');
      if (this.apiKey) {
    ...
    /**
     * @param {string} key - the raw API key value from the input field.
     * @param {boolean} remember - if true, persist in localStorage until
     *   explicitly cleared; if false (default), keep it in sessionStorage
     *   only, so it disappears when the tab closes.
     */
    setApiKey: function(key, remember = false) {
      if (!key || key.trim() === '') {
        this.clearStoredKey();
        return false;
      }
      ...
      if (remember) {
        ...
        StadiaAI.Utils.sessionStorage.remove('stadia_api_key');
      } else {
        StadiaAI.Utils.sessionStorage.set('stadia_api_key', this.apiKey);
        StadiaAI.Utils.storage.remove('stadia_api_key');
      }
      return true;
    },

    /** Wipes the key from memory and both storage backends. */
    ...
    /** True if the key currently in memory was persisted to localStorage. */
    isRemembered: function() {
      return StadiaAI.Utils.storage.get('stadia_api_key') !== null;
    },
