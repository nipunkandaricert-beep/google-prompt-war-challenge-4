(function() {
  ...
  StadiaAI.App = {
    ...
    init: function() {
      // 1. Check/initialize API Key
      this.checkApiKey();

      // 2. Initialize child modules
    ...
    checkApiKey: function() {
      const key = StadiaAI.Utils.storage.get('stadia_api_key');
    ...
    setupModalListeners: function() {
      ...
      if (submitBtn && input) {
        StadiaAI.Utils.on(submitBtn, 'click', () => {
          ...
          if (StadiaAI.Gemini) {
            StadiaAI.Gemini.setApiKey(val);
          }
          if (modal) modal.style.display = 'none';
          StadiaAI.Utils.showToast('Gemini API connected successfully!', 'success');
        ...
        card.innerHTML = `
          ...
          <div class="match-card-teams">
            <div class="match-team">
              <span class="match-flag">${m.team1.flag}</span>
              <span>${m.team1.code}</span>
            </div>
            <span class="match-vs">VS</span>
            <div class="match-team">
              <span>${m.team2.code}</span>
              <span class="match-flag">${m.team2.flag}</span>
            </div>
          </div>
