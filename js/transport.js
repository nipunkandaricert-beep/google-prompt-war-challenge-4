        ...
        transits.forEach((t, index) => {
          ...
          card.innerHTML = `
            <div class="transport-card-header">
              ...
              <div>
                <strong>${StadiaAI.Utils.escapeHtml(t.name)}</strong>
                <span class="badge ${t.status.includes('delay') ? 'badge-warning' : 'badge-success'}" style="margin-top: 4px; display:inline-block">${StadiaAI.Utils.escapeHtml(t.status)}</span>
              </div>
            </div>
            <p class="text-secondary text-sm">${StadiaAI.Utils.escapeHtml(t.route)}</p>
            <div class="transport-stats">
              ...
              <div>
                ...
                <span class="font-mono text-sm">${StadiaAI.Utils.escapeHtml(t.travelTime)}</span>
              </div>
              <div>
                <span class="text-muted text-xs block">Frequency</span>
                <span class="font-mono text-sm">${StadiaAI.Utils.escapeHtml(t.frequency)}</span>
              </div>
            </div>
          `;
        ...
        parkings.forEach((p, index) => {
          ...
          card.innerHTML = `
            <div class="transport-card-header">
              ...
              <div>
                <strong>${StadiaAI.Utils.escapeHtml(p.name)}</strong>
                <span class="badge badge-info" style="margin-top: 4px; display:inline-block">${StadiaAI.Utils.escapeHtml(p.price)} Flat Fare</span>
              </div>
            </div>
            ...
            <div class="transport-stats">
              <div>
                ...
                <span class="font-mono text-sm">${StadiaAI.Utils.escapeHtml(p.distance)} to Gates</span>
              </div>
              <div>
        ...
        rideshares.forEach((r, index) => {
          ...
          card.innerHTML = `
            <div class="transport-card-header">
              ...
              <div>
                <strong>${StadiaAI.Utils.escapeHtml(r.name)}</strong>
                <span class="badge badge-danger" style="margin-top: 4px; display:inline-block">Surge: ${StadiaAI.Utils.escapeHtml(r.surgePricing)}</span>
              </div>
            </div>
            <p class="text-secondary text-sm" style="line-height:1.4;">${StadiaAI.Utils.escapeHtml(r.pickupInstructions)}</p>
            <div class="transport-stats">
              <div>
                <span class="text-muted text-xs block">Avg. Wait Time</span>
                <span class="font-mono text-sm font-bold text-red">${StadiaAI.Utils.escapeHtml(r.avgWait)}</span>
              </div>
              <div>
