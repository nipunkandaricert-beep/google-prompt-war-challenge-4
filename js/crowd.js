(function() {
  ...
  StadiaAI.Crowd = {
    ...
    updateStats: function() {
      ...
      if (attEl) {
        const rawText = (attEl.innerText || attEl.textContent || '').replace(/,/g, '');
        const prevValue = parseInt(rawText) || 0;
      ...
      if (densEl) densEl.textContent = `${avgDensity}%`;

      // Update peak zone
      const peakEl = StadiaAI.Utils.$('#crowd-peak-zone');
      if (peakEl) peakEl.textContent = peakZone.name;
    },
