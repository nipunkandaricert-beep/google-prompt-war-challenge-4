    ...
    renderZoneTable: function() {
      ...
      zones.forEach(zone => {
        ...
        tr.innerHTML = `
          ...
          <td style="text-transform: capitalize;"><span class="text-secondary">${zone.type}</span></td>
      ...
      alerts.forEach(alert => {
        ...
        item.innerHTML = `
          <div class="alert-item-header">
            <span class="badge ${sevClass}" style="text-transform: uppercase;">${alert.severity}</span>
            <span class="text-muted text-xs">${StadiaAI.Utils.formatRelativeTime(alert.timestamp)}</span>
          </div>
          <p style="font-size: 0.875rem; color: var(--text-primary);">${alert.message}</p>
          <div style="display:flex; justify-content:space-between; align-items:center; margin-top: var(--space-2); border-top: 1px solid var(--border-subtle); padding-top: var(--space-2);">
            <span class="text-muted text-xs">📍 ${zoneName}</span>
            <button class="btn btn-ghost btn-sm btn-view-map" data-zone="${alert.zone}">View Map</button>
          </div>
        `;
