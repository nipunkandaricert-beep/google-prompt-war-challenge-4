(function() {
  ...
  StadiaAI.Navigation = {
    ...
    populateSelects: function() {
      ...
      zones.forEach(z => {
        const optionHtml = `<option value="${StadiaAI.Utils.escapeHtml(z.id)}">${StadiaAI.Utils.escapeHtml(z.name)} (L${z.level})</option>`;
      ...
      if (stepsList) {
        ...
        stepData.steps.forEach(step => {
          ...
          stepCard.innerHTML = `
            ...
            <div class="nav-step-text">${StadiaAI.Utils.escapeHtml(step.instruction)}</div>
            <div class="nav-step-dist">${StadiaAI.Utils.escapeHtml(String(step.distance))}m</div>
          `;
      ...
      if (aiTip) {
        ...
        try {
          ...
          aiTip.innerHTML = `
            ...
            <p class="text-sm" style="line-height: 1.5">${StadiaAI.Utils.escapeHtml(aiAdvice)}</p>
          `;
        } catch {
