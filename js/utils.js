(function() {
  ...
  StadiaAI.Utils = {
    ...
    // Security: escape untrusted text before it is ever interpolated into innerHTML.
    // Use this on (a) anything the user typed (chat input) and (b) anything the
    // Gemini API returned, before running it through parseMarkdown or inserting it directly.
    escapeHtml: (str) => {
    ...
    // Crowd density classification — single source of truth for the
    // 50/70/85% breakpoints, previously duplicated (with slightly different
    // scales) across crowd.js and stadium-map.js.
    CROWD_THRESHOLDS: { CRITICAL: 85, HIGH: 70, MEDIUM: 50 },

    /**
     * Classify a crowd density percentage into a severity level.
     * @param {number} percent - occupancy as a 0-100 value (not 0-1).
     * @returns {'critical'|'high'|'medium'|'low'}
     */
    getDensityLevel: (percent) => {
      const { CRITICAL, HIGH, MEDIUM } = StadiaAI.Utils.CROWD_THRESHOLDS;
    ...
    // SessionStorage wrapper — same interface as `storage`, but cleared when
    // the browser tab closes. Used for anything (like the Gemini API key)
    // that shouldn't persist on a shared device unless the user opts in.
    sessionStorage: {
      get: (key) => {
        try {
      ...
      remove: (key) => {
        try {
          ...
          console.error('Error deleting sessionStorage key', e);
        }
      }
    },
    ...
    showToast: (message, type = 'info', duration = 3000) => {
      ...
      toast.innerHTML = `${icon}<span>${StadiaAI.Utils.escapeHtml(message)}</span>`;
