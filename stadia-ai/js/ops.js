(function() {
  ...
  StadiaAI.Ops = {
    ...
    renderTimeline: function() {
      ...
      milestones.forEach(m => {
        ...
        item.innerHTML = `
          ...
          <span class="ops-milestone-title">${m.name}</span>
        `;
        ...
        card.innerHTML = `
          <div class="ops-alert-header">
            <div style="display:flex; align-items:center; gap:var(--space-2)">
              ${iconSvg}
              <strong style="text-transform: capitalize;">${alert.type} Incident</strong>
            </div>
            <span class="badge badge-danger text-xs">${alert.severity.toUpperCase()}</span>
          </div>
          <div class="ops-alert-body">${alert.message}</div>
          <div class="ops-alert-footer">
            <span class="text-secondary text-sm">📍 ${zoneName} • ${StadiaAI.Utils.formatRelativeTime(alert.timestamp)}</span>
    ...
    getAIDecision: async function(alert) {
      ...
      content.innerHTML = `
        <h4 style="font-size:0.9375rem; margin-bottom:var(--space-2);">Analyzing incident in ${StadiaAI.Data.getZone(alert.zone)?.name || 'stadium'}...</h4>
...
Provide an expert 3-step action plan for security or stadium staff to execute. Set priority level and state estimated resolution time. Keep it tactical, concise, and structured as numbered points. Under 80 words.`;
    ...
    renderResources: function() {
      ...
      resources.forEach(r => {
        ...
        card.innerHTML = `
          <span class="resource-label">${r.name}</span>
          <span class="resource-status" style="color: ${r.color}">${r.status}</span>
        `;
